...
constexpr const std::array<word, 11> Rcon = {0x00000000, 0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000, 0x20000000, 0x40000000, 0x80000000, 0x1b000000, 0x36000000};
    ...
    constexpr const std::array<byte, 256> INV_S_BOX = {
     ...
     */
    void pad_plaintext(std::vector<byte>& plaintext_bytes);
     ...
     * @param plaintext_bytes: Vector containing the bytes of the plaintext
     */
    void create_blocks(std::vector<std::vector<byte> >& plaintext_blocks,std::vector<byte> plaintext_bytes);
